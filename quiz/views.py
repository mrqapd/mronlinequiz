...
def admin_dashboard_view(request):
    ...
    'total_question':MModel.MQuestion.objects.all().count(),
    }
...
def view_question_view(request,pk):
    questions=MModel.MQuestion.objects.all().filter(course_id=pk)
    return render(request,'quiz/view_question.html',{'questions':questions})

@login_required(login_url='adminlogin')
def delete_question_view(request,pk):
    question=MModel.MQuestion.objects.get(id=pk)
    question.delete()
    return HttpResponseRedirect('/admin-view-question')
