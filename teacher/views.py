...
def teacher_check_marks_view(request,pk):
    ...
    if course.course_name == 'MBTI':
        ...
        return render(request,'teacher/teacher_check_marks_mbti.html',{'mresults':mresults})
    elif course.course_name == 'TATEST':
        tresults= TModel.Result_ta.objects.all().filter(exam=course).filter(student=student)
        return render(request,'teacher/teacher_check_marks_tatest.html',{'tresults' :tresults})
